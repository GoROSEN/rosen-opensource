...
type Plot struct {
	...
	OriginalMintPrice uint64            `gorm:"comment:Mint一次价格（原价）"`
	MaxMintPrice      uint64            `gorm:"default:0;comment:Mint最高价格"`
	MinMintPrice      uint64            `gorm:"default:0;comment:Mint最低价格"`
	...
	Blazer            *MemberExtra      `gorm:"foreignKey:BlazerID;reference:MemberID"`
	CoBlazer          *MemberExtra      `gorm:"foreignKey:CoBlazerID;reference:MemberID"`
	Listing           *ListingPlot      `gorm:"reference:PlotID"`
	MintLimitDuration uint              `gorm:"default:0;comment:mint限制周期（天），0不限"`
	MintLimitCount    uint              `gorm:"default:0;comment:周期内mint次数限制，0不限"`
	MintPriceByChains map[string]uint64 `gorm:"serializer:json;comment:一链一价"` // [chain]price
	Games             []Game            `gorm:"many2many:rosen_plot_games;"`
	ShowFrame         bool              `gorm:"default:true;comment:地块外框是否可见"`
}
...
type MintLog struct {
	...
	AssetID     uint   `gorm:"index;comment:AssetID"`
	MetaFileURL string `gorm:"size:2048;comment:MetaFileURL"`
	ToAddress   string `gorm:"size:128;"`
...
type MteSession struct {
	gorm.Model
	MemberExtraID uint         `gorm:"column:member_id;index;comment:会员ID"`
	...
	RosenMember   *MemberExtra `gorm:"foreignKey:MemberID;references:MemberExtraID"`
	Equip         *Asset       `gorm:"foreignKey:ID;references:EquipID"`
}
...
type MteTrace struct {
	...
	Latitude      float64      `gorm:"index;comment:纬度"`
	Longitude     float64      `gorm:"index;comment:经度"`
	RosenMember   *MemberExtra `gorm:"foreignKey:MemberExtraID;references:MemberID"`
}
...
type PlotCollection struct {
	...
	PlotID               int    `gorm:"index;comment:地块ID"`
	MintAccount          string `gorm:"size:256;comment:mint账户"`
	TokenAccount         string `gorm:"size:256;comment:ata账户"`
	MetadataAccount      string `gorm:"size:256;comment:mda账户"`
	MasterEditionAccount string `gorm:"size:256;comment:mea账户"`
}

func (PlotCollection) TableName() string {
	return "rosen_plot_collections"
}

type CityRank struct {
	gorm.Model

	City      string         `gorm:"index;size:256;comment:城市名称"`
	BlazerID  uint           `gorm:"index;comment:BlazerID"`
	PlotID    uint           `gorm:"index;comment:地块ID"`
	ExpiredAt time.Time      `gorm:"comment:失效时间"`
	Blazer    *member.Member `gorm:"foreignKey:ID;references:BlazerID"`
	Plot      *Plot
}

func (CityRank) TableName() string {
	return "rosen_cities_ranks"
}
