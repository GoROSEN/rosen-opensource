...
export default class PhantomWallet {
  ...
  constructor(network: string) {
    if (!network) {
      throw new Error(
        'a network(devnet,testnet,mainnet-beta, etc...) must be specified.',
      )
    }
  ...
  private unregisterDeepLinkHandler = (id: string): boolean =>
    this.phantomCallbackMap.delete(id)
  ...
  private unregisterOldDeepLinkHandlers = (minutes: number = 5) => {
    ...
    const now = new Date()
    this.phantomCallbackMap.forEach(
      (k, registeredHandler: RegisteredCallbackHandler) => {
        if (Math.abs(now - registeredHandler.date) > minutes) {
          //delete if more than a x minutes has passed
          keysToDelete.push(k)
        }
      },
    )
  ...
  private callDeepLinkMethod = (
    ...
    const params = new URLSearchParams({
      dapp_encryption_public_key: bs58.encode(
        this.phantomSession.keypair.publicKey,
      ), //this is not the wallet public key, this is used for key exchange encryption
    })
    ...
    const callbackLink = Linking.createURL(`${deepLinkReturnRoute}`, {
      queryParams: { id: handlerId, method },
    })
    ...
    return new Promise<PublicKey>(async (resolve, reject) => {
      try {
        ...
        if (this.phantomSession.token && !force) {
          reject(
            'already have a phantom session. set force=true to get a new session',
          )
          return
        }

        this.callDeepLinkMethod(
          DeepLinkMethod.connect,
          null,
          { resolve, reject },
          deepLinkReturnRoute,
        )
  ...
  signTransaction = async (
    ...
    return new Promise<Transaction>(async (resolve, reject) => {
      ...
      this.callDeepLinkMethod(
        DeepLinkMethod.signTransaction,
        payload,
        { resolve, reject },
        deepLinkReturnRoute,
      )
  ...
  signAndSendTransaction = async (
    ...
    return new Promise<string>(async (resolve, reject) => {
      ...
      this.callDeepLinkMethod(
        DeepLinkMethod.signAndSendTransaction,
        payload,
        { resolve, reject },
        deepLinkReturnRoute,
      )
  ...
  signAllTransactions = async (
    ...
    return new Promise<Transaction[]>(async (resolve, reject) => {
      ...
      const serializedTransactions = transactions.map(t =>
        bs58.encode(t.serialize({ requireAllSignatures, verifySignatures })),
      )
      ...
      this.callDeepLinkMethod(
        DeepLinkMethod.signAllTransactions,
        payload,
        { resolve, reject },
        deepLinkReturnRoute,
      )
